...
if __name__ == '__main__':
	...
	while(menu):
		...
		widget2.place(x=40,y=390)
		button_root = Button(root, text="Comprobar rooteo en el dispositivo", width=width_w, state=NORMAL, command=lambda: whatsapp_deb(root, 1))
		button_root.place(x=40, y=75)
		button_roote = Button(root, text="Rootear dispositivo", width=width_w, state=NORMAL, command=lambda: whatsapp_deb(root, 2))
		button_roote.place(x=40, y=125)
		button_mm = Button(root, text="Extracción de Whatsapp multimedia", width=width_w, state=NORMAL, command=lambda: whatsapp_deb(root, 3))
		button_mm.place(x=40, y=175)
		button_dbc = Button(root, text="Extracción de Data Base cifrada", width=width_w, state=NORMAL, command=lambda: whatsapp_deb(root, 4))
		button_dbc.place(x=40, y=225)
		button_db = Button(root, text="Extracción/Análisis de Data Base (root)", width=width_w, state=DISABLED, command=lambda: whatsapp_deb(root, 5))
		...
		if popup_a:
			...
			popup_a=False
		if label_root==True:
			label_Root = Label(root, text="Finished work.")
			label_Root.config(background="red")
			label_Root.place(x=270,y=355)
			label_root==False
